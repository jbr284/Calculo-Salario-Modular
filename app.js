// app.js - VERSÃO UNIFICADA (Funciona sem servidor)
...
const regras = {
    ...
    "tetoINSS": 8157.41,
    "percentualAdiantamento": 0.4,       // 40% (Regra Modular)
    "percentualAdicionalNoturno": 0.35,  // 35% (Regra Modular)
...
// --- 3. INTERFACE E LÓGICA DE EVENTOS ---
document.addEventListener('DOMContentLoaded', () => {
        ...
        resultContainer.innerHTML = `
            ...
            <table class="result-table">
                ...
                <tbody>
                    ...
                    ${row('Faltas/Atrasos', descontos.descontoFaltas + descontos.descontoAtrasos)}
    ...
    // --- LÓGICA DE FÉRIAS (Sua Versão Blindada Híbrida) ---
    function alternarModoDias() {
        ...
        // Trava visualmente o campo de dias a pagar
    ...
    function calcularDiasProporcionaisFerias() {
        ...
        // Limpa feedback antigo
        feedbackFerias.innerHTML = "";

        if (!mesRefStr) {
            diasTrabInput.value = 0;
            feedbackFerias.innerHTML = "Selecione o Mês de Referência.";
            return;
        }
        ...
        // LÓGICA HÍBRIDA (Sanduíche vs Saída Pura)
        if (modo === 'saida_ferias') {
            ...
            // Verifica quantos dias de férias caem neste mês
            ...
            if (dataFimFerias <= fimMes) {
                // Sanduíche: trabalhou antes e depois
                diasTrabalhados = 30 - diasFeriasNoMes;
                texto = "Sanduíche (Retornou ao trabalho)";
            } else {
                // Saída Pura: trabalhou até sair
                diasTrabalhados = diaValidado - 1;
                texto = "Saída (Não retornou no mês)";
            }
            ...
            feedbackFerias.innerHTML = `Retornou dia: <b>${dataRetorno.toLocaleDateString('pt-BR')}</b>.<br>Dias trabalhados: <b>${diasTrabalhados}</b>.`;
        }
    ...
    function handleCalcular() {
        // Função segura para pegar valor numérico
        const getVal = (id) => {
            const el = document.getElementById(id);
            if(!el) return 0;
            // Trata inputs vazios ou inválidos
            const v = parseFloat(el.value);
            return isNaN(v) ? 0 : v;
        };

        // Captura e Conversão de Inputs
        const inputs = {
    ...
    function adicionarFeriado() {
        ...
        div.onclick = () => {
            const novaLista = campo.value.split(',').filter(f => f !== data);
    ...
    function restaurarDadosFixos() {
        const dadosSalvos = JSON.parse(localStorage.getItem('dadosFixosModular'));
        if (dadosSalvos) {
            ...
            document.getElementById('plano').value = dadosSalvos.plano || "nenhum";
            document.getElementById('sindicato').value = dadosSalvos.sindicato || "nao";
        }
    }
    ...
    document.querySelectorAll('.hora-conversivel').forEach(campo => {
        campo.addEventListener('blur', function() {
            ...
            if(valor) this.value = parseFloat(valor).toFixed(2);
        });
    });
